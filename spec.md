...
- API: `GET /api/data` · `POST /api/refresh` · `POST /api/tasks` · `DELETE /api/tasks/:id` · `POST /api/state` (done, hiddenCourses, calendarHidden, seenAnnouncements, notes, notifications, features) · `POST /api/notify-test` · `POST /api/notification-settings` · `POST /api/signin` · Google Calendar (optional feature): `POST /api/gcal/{connect,sync,load,settings,event,from-item}`, see `spec-google-calendar.md` · Smart Announcements (optional feature): `POST /api/smart/{scan,decide}`, see `spec-smart-announcements.md`.
