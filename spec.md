...
## 5. Layout
- Header: Purdue logo (links to myPurdue in a new tab) · "Academic Dashboard" · last/next refresh · Refresh · ⚙ Settings (Courses · Notifications · Features, with an orange dot when something needs attention). Banner row only when something is wrong.
...
- API: `GET /api/data` · `POST /api/refresh` · `POST /api/tasks` · `DELETE /api/tasks/:id` · `POST /api/state` (done, hiddenCourses, calendarHidden, seenAnnouncements, notes, notifications, features, grades, updateSnooze) · `POST /api/notify-test` · `POST /api/notification-settings` · `POST /api/signin` · Google Calendar (optional feature): `POST /api/gcal/{connect,sync,load,settings,event,from-item}`, see `spec-google-calendar.md` · Smart Announcements (optional feature): `POST /api/smart/{scan,decide}`, see `spec-smart-announcements.md` · updates: `POST /api/update/check`, `POST /api/update` · Syllabus scan (optional feature): `POST /api/syllabus/{scan,decide,file,file-remove}`; see `spec-next-features.md`.
